...
export function StorageAnalyzer() {
  ...
  const insights = (() => {
    ...
    const parseBytes = (sizeStr: string) => {
      ...
      let bytes = Number.parseFloat(num)
      switch (unit) {
        case "KB":
          bytes *= 1024
          break
    ...
    const sorted = [...storageData].sort((a, b) => parseBytes(b.size) - parseBytes(a.size))
    const largest = sorted[0]
    const quickWins = storageData
      .filter((s) => s.canClear)
      .slice(0, 3)
      .map((s) => ({ type: s.type, size: s.size }))
    const hasHeavyCookies = storageData.find(
      (s) => s.type.toLowerCase().includes("cookie") && parseBytes(s.size) > 10 * 1024,
    )
    return { largest, quickWins, hasHeavyCookies: Boolean(hasHeavyCookies) }
  })()
